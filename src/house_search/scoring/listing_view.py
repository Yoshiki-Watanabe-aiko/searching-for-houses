...
class ListingView:
    ...
    # **表示専用**で採点にも MUST にも使わない。
    # ⚠ 掲載そのものの ``type_specific_attrs`` から読む（グループの和集合にはしない。
    #   建築条件は区画ごとに違いうるので、別の掲載の条件を混ぜると誤読の元になる）
    # ⚠ 除外の MUST は下の ``build_condition``（グループ全体から導く）が読む。別物
    caveats: tuple[str, ...] = ()
    # 建築条件付きかどうか（→ 課題#61）。土地の「建築条件付きを除く」MUST が読む。
    # ⚠ **グループ全体から導く**（1件でも付なら True）。代表は最安の掲載で、
    #   代表の判定だけで決めると代表の入れ替わりで同じ区画が出たり消えたりする。
    #   None ＝ 読めない（詳細未取得・``一部建築条件付``）で、MUST は unknown
