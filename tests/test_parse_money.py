...
class Test万と円が同居する表記:
    """⚠ ``1万3020円`` の下位桁が落ちていた（→ 課題#53）。

    SUUMO 中古マンションの詳細ページは管理費・修繕積立金を ``1万3020円／月``
    の形で書く。``_MAN_YEN`` は「万」の後ろの数字を見ないので **10,000** を返す。

    ⚠⚠ **過小に出る**ため管理費が安く見え、``monthly_cost`` の採点が甘くなる。
    課題#51（全角で管理費が0円）・課題#4（1億円ぶんが落ちる）と同じ
    「例外にも件数の減少にもならないまま値だけ狂う」形である。
    """

    @pytest.mark.parametrize(
        ("text", "expected"),
        [
            # SUUMO 中古マンション nc_21457575 の実表記（2026-09-06 実測）
            ("1万3020円／月", 13_020),
            ("1万900円／月", 10_900),
            ("10万5000円", 105_000),
            ("2万5,000円", 25_000),
    ...
    def test_万の後ろの円を落とさない(self, text: str, expected: int) -> None:
        assert parse_yen(text) == expected
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # ABLE の敷金欄は「円」を省く
            ("23.9万", 239_000),
            # ⚠ 「万円」の直後にある別の金額と結合しないこと
            ("15万円/月 管理費5000円", 150_000),
        ],
    )
    def test_万だけの表記は変わらない(self, text: str, expected: int) -> None:
        assert parse_yen(text) == expected

    def test_億と万と円が並んでも落ちない(self) -> None:
        """⚠ 実データでは未観測。同じ欠陥を三度繰り返さないために先に固定する。"""
        assert parse_yen("1億2800万3020円") == 128_003_020
        assert parse_yen("1億2,800万円") == 128_000_000

    def test_管理費の欄として読んでも同じ(self) -> None:
        assert parse_fee("1万3020円／月") == 13_020
