...
骨格（9a）と **SUUMO の土地アダプタ（9b）まで実装済み**（2026-09-11）。⚠ 設備の抽出辞書（9c）と
実運用パターン（9e）はまだ無いので、`configs/` 直下に土地のパターンは置いていない
...
（WANT なら全件 miss で分母にだけ乗る）。⚠ **土地の辞書はまだ空**なので、土地パターンには
設備条件を書かない。照合は WANT（`any_of` も展開）と MUST の両方を、辞書＋導出コード
