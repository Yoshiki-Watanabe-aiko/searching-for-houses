-- ============================================================
-- m_sites: サイトマスタ（18行 / 賃貸のスクレイピング対象は16サイト）
--
...
INSERT INTO m_sites (
    ...
    ('HOUSECOM',   'ハウスコム',       'https://www.housecom.jp', 'HTTP',      TRUE,  2.5, 5, NULL, 160, '仲介ポータル。市区スラグは都道府県索引1本で採れる（1都3県4本で224市区・帯82/82 → 詳細設計書 §13）。⚠ **既定の並びは1ページ目が高い住戸で埋まり MUST を1件も通さない**ので ?sort=0（家賃が安い順）を必ず付ける。⚠ sort=0 は棟あたり最安1住戸だけを返す。⚠ 間取りの複数指定はクエリ配列 cc_mdr[] でしかできない（パス形式の複数指定は404）。⚠ .property_build は棟で住戸は .property_room。⚠ **号室の伏字（***号室）は掲載終了ではなく募集中**（本番413掲載のうち100件＝24%が伏字）。掲載終了の判定は404のみ。2.5秒間隔で20市区を連続取得しても検知は出ない'),
    ('HOMEMATE',   'ホームメイト',     'https://www.homemate.co.jp', 'HTTP',   TRUE,  2.5, 5, NULL, 170, '仲介ポータル（東建コーポレーション系）。市区の検索値がJIS5桁そのものなのでスラグ収集が要らない（→ 詳細設計書 §13）。⚠ **robots.txt が User-agent: ClaudeBot を全面禁止**しているが、名乗るUAは house-search/2.0 なので User-agent: * の規則（/ad 等のみ Disallow）が適用される。`*` に従って取得するのはユーザー判断（2026-09-04 → §13.2）。⚠ ekiw（駅徒歩）は選択肢が15分までで MUST の20分を表現できないため配線しない。⚠ 階の表記が無い住戸が実在する。⚠ 物件番号に全角数字が混じりURLエンコードされる。掲載終了は素直に404。2.5秒間隔で20市区を連続取得しても検知は出ない'),
    ('CHINTAI_NET', 'CHINTAI.net',      'https://www.chintai.net', 'HTTP',      TRUE,  2.5, 5, NULL, 180, '仲介ポータル（CHINTAIブランド）。市区の検索値がJIS5桁そのものでスラグ収集が要らない（→ 詳細設計書 §14）。⚠⚠ **一覧に返る住戸が実行のたびに入れ替わる**ので、「返る掲載の中身でフィルタの効きを確かめる」手が使えず、総件数（本文の「N件」）だけが安定した判定材料だった。並び順を指定しても揺れは止まらないので o は送らない。⚠ .cassette_item は棟で、住戸は tbody[data-bkkey]（棟だけ数えると49住戸のうち26件を落とす）。⚠ PR枠は構造が違ううえ data-detailurl が robots 禁止の ?vm= を含むので取り込まない。⚠⚠ 詳細の .detail_specTable には用語集の解説文が展開されており、その住戸に無い設備名（レンジフード・IHクッキングヒーター）が辞書に拾われるので .mod_equipmentBox だけを使う。⚠ hidden input の chinRyo は「118000」の生値で parse_yen が読めない（素朴に書くとprice が全件 NULL になり MUST が unknown へ落ちる）。⚠ 都道府県ページは404で市区必須。サイト側フィルタは sf/ct/j/m の4軸すべて効く。掲載終了は素直に404。2.5秒間隔で20市区を連続取得しても検知は出ない。⚠ robots.txt が User-agent: * を2グループ持つので課題#43 の修正が前提')
