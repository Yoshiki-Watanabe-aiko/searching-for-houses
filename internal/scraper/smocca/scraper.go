...
const siteCode = "SMOCCA"
const baseURL = "https://smocca.jp"
const pageSize = 20

// URL format: /search/{prefSlug}/city/{jisCode}  (page 1)
//             /search/{prefSlug}/city/{jisCode}/page/{n}  (page 2+)
...
var reSmoccaID = regexp.MustCompile(`/bukken/detail/([^/?]+)`)
...
func (s *Scraper) Scrape(ctx context.Context, cfg *config.SearchConfig, fullScan bool, maxPages int) ([]*model.ScrapedProperty, error) {
	...
	if !ok {
		return nil, fmt.Errorf("unsupported prefecture for SMOCCA: %s", pref)
	}
	...
	for _, cityCode := range cities {
		if cityCode == "" {
			continue // prefecture-level search not supported; specify cities in YAML
		}
		base := fmt.Sprintf("%s/search/%s/city/%s", baseURL, slug, cityCode)
		extraParams := buildQueryParams(cfg)
...
func buildQueryParams(cfg *config.SearchConfig) string {
	...
	c := cfg.Conditions
	if c.Price.NoReikin {
		params.Set("cond[reikin]", "true")
	}
	if c.Price.NoShikikin {
		params.Set("cond[shikikin]", "true")
	}
...
func parseListings(doc *goquery.Document) []*model.ScrapedProperty {
	...
	detailLinks := doc.Find("a[href*='/bukken/detail/']")
	detailLinks.Each(func(_ int, a *goquery.Selection) {
		href, ok := a.Attr("href")
		if !ok {
			return
		}
		m := reSmoccaID.FindStringSubmatch(href)
		if len(m) < 2 {
			return
		}
