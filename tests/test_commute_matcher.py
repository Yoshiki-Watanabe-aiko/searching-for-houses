"""駅同定のテスト。

回帰データはすべて 2026-09-03 の実DB（active 10,322掲載）から採った実表記。
サイトごとの癖（全角ＪＲ・会社名の前置・区切り無しの連結・バス停の併記）を
そのまま焼き込んである。
"""

from __future__ import annotations

import pytest

from house_search.commute.matcher import (
    MATCH_AMBIGUOUS,
    MATCH_MATCHED,
    MATCH_UNMATCHED,
    StationIndex,
    candidate_variants,
    extract_station_names,
    match_stations,
    station_walk_minutes,
)
from house_search.commute.normalize import normalize_key

# 都道府県コード（JIS X 0401）
SAITAMA, CHIBA, TOKYO, KANAGAWA = 11, 12, 13, 14

# テスト用の索引。実マスタから必要な駅だけを抜き出したもの。
# 「浅草」は東京都内に2グループある（メトロ/都営/東武 と つくばエクスプレス）。
# 「小川町」は埼玉と東京にそれぞれあり、都道府県で絞らないと曖昧になる。
_STATIONS: tuple[tuple[str, int, int], ...] = (
    ("八王子", 1_001, TOKYO),
    ("京王八王子", 1_002, TOKYO),
    ("北千住", 1_003, TOKYO),
    ("六町", 1_004, TOKYO),
    ("北綾瀬", 1_005, TOKYO),
    ("青井", 1_006, TOKYO),
    ("竹ノ塚", 1_007, TOKYO),
    ("押上〈スカイツリー前〉", 1_008, TOKYO),
    ("明治神宮前〈原宿〉", 1_009, TOKYO),
    ("鉄道博物館", 1_010, SAITAMA),
    ("川間", 1_011, CHIBA),
    ("千葉", 1_012, CHIBA),
    ("本納", 1_013, CHIBA),
    ("成田空港（第１旅客ターミナル）", 1_014, CHIBA),
    ("元町・中華街", 1_015, KANAGAWA),
    ("上永谷", 1_016, KANAGAWA),
    ("青堀", 1_017, CHIBA),
    ("君津", 1_018, CHIBA),
    ("木更津", 1_019, CHIBA),
    ("指扇", 1_020, SAITAMA),
    ("大宮", 1_021, SAITAMA),
    ("高尾", 1_022, TOKYO),
    ("小川町", 1_023, SAITAMA),
    ("小川町", 1_024, TOKYO),
    ("浅草", 1_025, TOKYO),
    ("浅草", 1_026, TOKYO),
    ("番田", 1_027, KANAGAWA),
)


@pytest.fixture(scope="module")
def index() -> StationIndex:
    return StationIndex.build(
        (normalize_key(name), group_code, pref) for name, group_code, pref in _STATIONS
    )


def matched_codes(info: str, index: StationIndex, pref: int | None = None) -> list[int]:
    return [
        m.station_g_cd for m in match_stations(info, index, pref) if m.match_status == MATCH_MATCHED
    ]


# --- サイトごとの実表記 ---------------------------------------------------


@pytest.mark.parametrize(
    ("site", "info", "pref", "expected"),
    [
        (
            "SUUMO",
            "ＪＲ中央線/八王子駅 バス18分 (バス停)滝山城址 歩2分",
            TOKYO,
            [1_001],
        ),
        (
            "ABLE",
            "東武野田線<アーバンパークライン>/川間駅 バス20分:停歩8分 "
            "東武伊勢崎線・スカイツリーライン/竹ノ塚駅 徒歩10分",
            CHIBA,
            [1_011, 1_007],
        ),
        (
            "APAMAN",
            "ＪＲ総武本線 千葉駅/バス乗車30分/千葉中央バス㈱ 大和田入口/徒歩2分",
            CHIBA,
            [1_012],
        ),
        (
            "HOMES",
            "JR内房線 青堀駅 バス4分 下安知郡下車 徒歩2分 / "
            "JR内房線 君津駅 バス24分 イオンモール富津下車 徒歩17分",
            CHIBA,
            [1_017, 1_018],
        ),
        (
            "EHEYA",
            "みなとみらい線 元町・中華街（山下公園）駅 バス8分 本牧１丁目 徒歩4分",
            KANAGAWA,
            [1_015],
        ),
        (
            "NIFTY",
            "指扇駅 バス8分 歩4分 （川越線） / 大宮駅 バス8分 歩4分 （東北新幹線など）",
            SAITAMA,
            [1_020, 1_021],
        ),
        (
            # goo は路線名と駅名が地続きになり、距離が km 表記になる
            "GOO",
            "ＪＲ外房線本納駅徒歩8000m",
            CHIBA,
            [1_013],
        ),
        (
            # スモッカは「駅」が付かず区切りも無い（第2パス）
            "SMOCCA",
            "東京地下鉄千代田線/北綾瀬 徒歩8分つくばエクスプレス/六町 徒歩14分"
            "つくばエクスプレス/青井 徒歩16分",
            TOKYO,
            [1_005, 1_004, 1_006],
        ),
        (
            # ⚠ ATHOME は駅名を鉤括弧で囲む。囲みを外さないと「駅」の直前が「」に
            # なり第1パスが1件も拾えず、**同定結果が0件**になる（→ 2026-09-04）
            "ATHOME",
            "ＪＲ京浜東北線 「千葉」駅 徒歩10分",
            CHIBA,
            [1_012],
        ),
        (
            # UR の交通欄は半角の鉤括弧で、路線名と地続きになる
            "UR",
            "JR中央線｢高尾｣駅バス7分 徒歩1～11分",
            TOKYO,
            [1_022],
        ),
        (
            # 賃貸EX も「駅」なし。バス停が「から」で続く
            "CHINTAI_EX",
            "高尾 バス6分 元八王子2丁目バス停から徒歩4分八王子 バス28分 "
            "城山手バス停から徒歩5分京王八王子 バス30分",
            TOKYO,
            [1_022, 1_001, 1_002],
        ),
    ],
)
def test_サイトごとの実表記から駅を同定できる(
    site: str, info: str, pref: int, expected: list[int], index: StationIndex
) -> None:
    assert matched_codes(info, index, pref) == expected, site


# --- バス停を駅として拾わない ---------------------------------------------


@pytest.mark.parametrize(
    ("info", "pref"),
    [
        # 「(バス停)一之江五丁目」「(バス停)栄町（神奈川県）」のような紛らわしい名前
        ("ＪＲ中央線/八王子駅 バス18分 (バス停)栄町（神奈川県） 歩2分", TOKYO),
        # APAMAN のバス会社＋バス停
        ("ＪＲ総武本線 千葉駅/バス乗車30分/小湊バス㈱ 千葉県がんセンター/徒歩20分", CHIBA),
        # goo の「」つきバス停
        ("ＪＲ中央線 八王子駅 バス25分/「品の木・ハイランドホテル前」バス停 停歩9分", TOKYO),
        # いい部屋ネットのバス停（バス分数と徒歩の間に挟まる）
        ("ブルーライン 上永谷駅 バス9分 神奈川中央交通バス吉原小学校前 徒歩6分", KANAGAWA),
    ],
)
def test_バス停の名前を駅として拾わない(info: str, pref: int, index: StationIndex) -> None:
    with_suffix, _ = extract_station_names(info)
    matches = match_stations(info, index, pref)
    # 「◯◯駅」で拾えた候補以外が混ざっていないこと
    assert [m.raw_name for m in matches if m.match_status == MATCH_MATCHED] == list(with_suffix)


# ⚠ 原文はすべて本番DBの active 掲載から採ったもの（2026-09-15・SUUMO 売買）。
# SUUMO は**バス停も「「◯◯」駅」の形で書く**。直前の語はバス会社名・出発駅で、
# 路線名ではない。駅として拾うとバス停からの徒歩が駅徒歩になり、
# 土地の上位15件に3件入っていた（1位は「川越観光 「高坂ニュータウン入口」駅 徒歩3分」）。
@pytest.mark.parametrize(
    ("info", "names", "walks"),
    [
        ("神奈川中央交通 「平和町」駅 徒歩4分", (), {}),
        ("川越観光 「高坂ニュータウン入口」駅 徒歩3分", (), {}),
        ("神奈中 「太井」駅 徒歩7分", (), {}),
        ("京急 「ハイランド入口」駅 徒歩3分", (), {}),
        # ⚠ 「直後がバスなら出発駅」は「駅」の直後で見る（徒歩の後ろのバスに引きずられない）
        ("京急 「ハイランド入口」駅 徒歩3分 バス便あり", (), {}),
        ("日東交通 「畑沢南五丁目」駅 徒歩7分", (), {}),
        ("JRバス関東 「三里塚小学校」駅 徒歩7分", (), {}),
        ("朝日自動車株式会社 「府内」駅 徒歩7分", (), {}),
        ("しもうさ循環ルート 「外記林入口」駅 徒歩10分", (), {}),
        ("京成バス 草野車庫 「宮の杜中央」駅 徒歩4分", (), {}),
        # ⚠ 徒歩がレンジ。後ろの「～3分」を残すと第2パスが拾いうる
        ("京成バス千葉ウエスト 「北谷津川」駅 徒歩2分～3分", (), {}),
        # 直前の語が出発駅。出発駅は駅として残し、バス停だけを消す
        ("茅ヶ崎駅 「芹沢入口」駅 徒歩8分", ("茅ヶ崎",), {}),
        ("西武新宿線 新狭山駅 「柏原北」駅 徒歩1分", ("新狭山",), {}),
        ("上大岡駅バス10分 「普門院前停」駅 徒歩7分", ("上大岡",), {}),
        ("西武線 「航空公園」駅 駅バス6分 「北原町中央」駅 徒歩4分", ("航空公園",), {}),
        # ⚠ 「小田急」はバス会社だが、直後がバスなら出発駅（海老名）なので残す
        ("小田急 「海老名」駅 駅よりバス12分 「中原」駅 徒歩3分", ("海老名",), {}),
    ],
)
def test_鉤括弧つきのバス停を駅として拾わない(
    info: str, names: tuple[str, ...], walks: dict[str, int]
) -> None:
    assert extract_station_names(info)[0] == names
    assert station_walk_minutes(info) == walks


@pytest.mark.parametrize(
    ("info", "names", "walks"),
    [
        # 直前の語が路線名なら駅（…線／…ライン／…鉄道／…モノレール／…ライナー など）
        ("ＪＲ京浜東北線 「北浦和」駅 徒歩10分", ("北浦和",), {"北浦和": 10}),
        ("湘南新宿ライン宇須 「東大宮」駅 徒歩12分", ("東大宮",), {"東大宮": 12}),
        ("埼玉高速鉄道 「浦和美園」駅 徒歩9分", ("浦和美園",), {"浦和美園": 9}),
        ("多摩モノレール 「立飛」駅 徒歩6分", ("立飛",), {"立飛": 6}),
        ("成田スカイアクセス 「成田湯川」駅 徒歩15分", ("成田湯川",), {"成田湯川": 15}),
        ("日暮里舎人 「見沼代親水公園」駅 徒歩8分", ("見沼代親水公園",), {"見沼代親水公園": 8}),
        ("つくばＥＸ 「六町」駅 徒歩10分", ("六町",), {"六町": 10}),
        ("ゆりかもめ 「お台場海浜公園」駅 徒歩5分", ("お台場海浜公園",), {"お台場海浜公園": 5}),
        # 直前が区切り（括弧）で語が空なら駅
        ("小田急線（新宿～相模大野） 「喜多見」駅 徒歩15分", ("喜多見",), {"喜多見": 15}),
        # D-room・UR（路線名と地続き）
        ("常磐緩行線 「綾瀬」駅徒歩8分", ("綾瀬",), {"綾瀬": 8}),
        ("JR中央線｢高尾｣駅バス7分", ("JR中央線高尾",), {}),
    ],
)
def test_鉤括弧つきの駅は路線名が前にあれば駅のまま(
    info: str, names: tuple[str, ...], walks: dict[str, int]
) -> None:
    assert extract_station_names(info)[0] == names
    assert station_walk_minutes(info) == walks


# --- 表記ゆれの吸収 -------------------------------------------------------


@pytest.mark.parametrize(
    ("info", "expected_code"),
    [
        ("東武伊勢崎線 押上駅 徒歩5分", 1_008),  # マスタ側に副名称〈スカイツリー前〉
        ("東京メトロ千代田線 明治神宮前駅 徒歩3分", 1_009),  # 同上〈原宿〉
        ("京成本線 成田空港駅 徒歩5分", 1_014),  # マスタ側に（第１旅客ターミナル）
        ("みなとみらい線 元町中華街駅 徒歩4分", 1_015),  # 掲載側に中黒が無い
        ("ＪＲ中央線/八王子駅 歩4分", 1_001),  # 全角ＪＲ
    ],
)
def test_副名称や中黒の違いを吸収して同定する(
    info: str, expected_code: int, index: StationIndex
) -> None:
    assert matched_codes(info, index) == [expected_code]


def test_路線名を剥がす規則が実在の駅名を壊さない(index: StationIndex) -> None:
    """「鉄道博物館」は路線名の接頭辞を剥がす規則に引っかかるが、原文を先に試すので壊れない。"""
    assert candidate_variants("鉄道博物館")[0] == "鉄道博物館"
    assert matched_codes("ニューシャトル 鉄道博物館駅 徒歩5分", index, SAITAMA) == [1_010]


# --- 都道府県スコープ -----------------------------------------------------


def test_都道府県で絞ると同名駅の曖昧さが解ける(index: StationIndex) -> None:
    """「小川町」は埼玉（東武東上線）と東京（都営新宿線）にある。"""
    assert matched_codes("東武東上線/小川町駅 徒歩3分", index, SAITAMA) == [1_023]
    assert matched_codes("都営新宿線/小川町駅 徒歩3分", index, TOKYO) == [1_024]


def test_都道府県が不明なら同名駅は曖昧のままにする(index: StationIndex) -> None:
    matches = match_stations("小川町駅 徒歩3分", index, None)
    assert [m.match_status for m in matches] == [MATCH_AMBIGUOUS]
    assert matches[0].station_g_cd is None


def test_同一都道府県内で割れる駅は曖昧にする(index: StationIndex) -> None:
    """浅草は東京都内に2グループある（東武ほか / つくばエクスプレス）。

    路線名でも座標でも絞らない。実測で ambiguous は 10,322掲載中23件しかなく、
    大半は同じ掲載の別の駅で補える（→ ADR 0016）。
    """
    matches = match_stations("東武伊勢崎線 浅草駅 徒歩8分", index, TOKYO)
    assert [m.match_status for m in matches] == [MATCH_AMBIGUOUS]


def test_県外の駅は同定できないと記録する(index: StationIndex) -> None:
    matches = match_stations("上越新幹線/本庄早稲田駅 徒歩13分", index, SAITAMA)
    assert [(m.raw_name, m.match_status) for m in matches] == [
        ("本庄早稲田", MATCH_UNMATCHED)
    ]


# --- 第2パスの安全側の扱い ------------------------------------------------


def test_駅の接尾辞が無い候補はマスタに当たったものだけ採る(index: StationIndex) -> None:
    """アンカーが無い第2パスは推測なので、外れた候補を記録に残さない。"""
    matches = match_stations("愛川・高峰ルート/半原小学校入口 歩7分", index, KANAGAWA)
    assert matches == ()


def test_駅の接尾辞がある候補は外れても記録する(index: StationIndex) -> None:
    """「◯◯駅」で拾えた候補は、同定に失敗しても規則を直す材料として残す。"""
    matches = match_stations("秩父鉄道/広瀬川原駅 徒歩5分", index, SAITAMA)
    assert [(m.raw_name, m.match_status) for m in matches] == [("広瀬川原", MATCH_UNMATCHED)]


def test_駅情報が空なら何も返さない(index: StationIndex) -> None:
    assert match_stations(None, index) == ()
    assert match_stations("   ", index) == ()


def test_同じ駅が複数回出ても1件にまとめる(index: StationIndex) -> None:
    info = (
        "東武伊勢崎線/竹ノ塚駅 歩15分 / 東武伊勢崎線/竹ノ塚駅 バス5分 (バス停)弁天町 歩3分"
    )
    assert matched_codes(info, index, TOKYO) == [1_007]


# --- 照合キーの正規化 -----------------------------------------------------


@pytest.mark.parametrize(
    ("left", "right"),
    [
        ("押上〈スカイツリー前〉", "押上駅"),
        ("成田空港（第１旅客ターミナル）", "成田空港"),
        ("霞ヶ関", "霞ケ関"),  # 小書き仮名（鎌ケ谷市で踏んだのと同じ揺れ）
        ("一之江", "一ノ江"),
        ("元町・中華街", "元町中華街"),
        ("ＪＲ相模線", "JR相模線"),  # 全角英字
        ("センター北", "センタ-北"),  # 長音とハイフン
    ],
)
def test_表記が違っても同じ照合キーになる(left: str, right: str) -> None:
    assert normalize_key(left) == normalize_key(right)


@pytest.mark.parametrize(
    ("left", "right"),
    [
        ("八王子", "京王八王子"),
        ("大宮", "西大宮"),
        ("日ノ出", "日ノ出町"),
    ],
)
def test_別の駅は別の照合キーになる(left: str, right: str) -> None:
    assert normalize_key(left) != normalize_key(right)


# ⚠ 原文はすべて本番DBの active 掲載から採ったもの（2026-09-07）。
# 「バス停からの徒歩を駅徒歩にしない」ことがこの一群の目的で、
# 実測では walk_minutes が 3,090件でバス停徒歩になっていた（→ 課題#58）。
@pytest.mark.parametrize(
    ("info", "expected"),
    [
        # 駅徒歩とバス便が混在する。バス便の駅は含めない
        (
            "ＪＲ中央線/武蔵小金井駅 歩5分 / 西武多摩川線/新小金井駅 バス12分 "
            "(バス停)中町4丁目 歩2分 / 京王線/府中駅 バス17分 (バス停)前原坂 歩2分",
            {"武蔵小金井": 5},
        ),
        # 全駅が駅徒歩
        (
            "小田急線/柿生駅 歩18分 / 小田急線/鶴川駅 歩30分 / 小田急多摩線/五月台駅 歩44分",
            {"柿生": 18, "鶴川": 30, "五月台": 44},
        ),
        # エイブル: 「バス:13分:停歩4分」の4分はバス停から
        (
            "東急田園都市線/藤が丘駅 徒歩18分 東急田園都市線/市が尾駅 徒歩18分 "
            "東急田園都市線/青葉台駅 バス:13分:停歩4分",
            {"藤が丘": 18, "市が尾": 18},
        ),
        # APAMAN: 「/バス乗車10分/諏訪坂/徒歩2分」の2分はバス停から
        (
            "ＪＲ埼京線 与野本町駅/徒歩19分 / 京浜東北・根岸線 北浦和駅/バス乗車10分/"
            "諏訪坂/徒歩2分 / 京浜東北・根岸線 与野駅/徒歩34分",
            {"与野本町": 19, "与野": 34},
        ),
        # ATHOME: 鉤括弧つきの駅名。後ろのバス表記に引きずられない
        (
            "ＪＲ南武線 「鹿島田」駅 徒歩13分 [バス利用可] バス 1分 上平間（川崎市） 停歩2分",
            {"鹿島田": 13},
        ),
        # HOME'S: 「下車 徒歩1分」はバス停から
        (
            "都営新宿線 篠崎駅 徒歩14分 / 都営新宿線 瑞江駅 バス9分 小岩消防署下車 徒歩1分",
            {"篠崎": 14},
        ),
        # goo: 先頭の駅がバス便
        (
            "小田急線 新百合ヶ丘駅 バス5分/「千代ヶ丘」バス停 停歩1分 / "
            "小田急線 百合ヶ丘駅 徒歩20分 / 小田急線 読売ランド前駅 徒歩25分",
            {"百合ヶ丘": 20, "読売ランド前": 25},
        ),
        # ニフティ: 「バス10分 歩8分」の8分はバス停から
        ("柏駅 バス10分 歩8分 （常磐線など） / 北柏駅 歩28分 （常磐緩行線）", {"北柏": 28}),
        # ⚠ いい部屋ネット: mask_bus_stops が「バス7分 とのした橋 」ごと消すので、
        # マスク後の文字列で見ると「溝の口駅 徒歩2分」に見えてしまう
        ("東急田園都市線 溝の口駅 バス7分 とのした橋 徒歩2分", {}),
        # D-room: 鉤括弧つき・全駅がバス便
        (
            "埼京線 「与野本町」駅 バス15分「白鍬」停徒歩3分 京浜東北線 「北浦和」駅 "
            "バス22分「白鍬」停徒歩3分",
            {},
        ),
        # ホームメイト: 駅名の直後が「まで14分」で徒歩ではない
        ("バス 中宗岡停まで徒歩3分、バス乗車して東武東上線 志木駅まで14分", {}),
        # ⚠ ハウスコム: 「柏駅 大井まで徒歩6分」は駅から徒歩6分ではない（大井は地名）
        (
            "常磐線 柏駅 大井まで徒歩6分、駅までバスで15分 "
            "東武アーバンパークライン・野田線 増尾駅 4.16km",
            {},
        ),
        # UR: 交通欄に時間表記が無い
        ("秋山駅 / 市川大野駅 / 松戸駅", {}),
        # スモッカ: 「駅」の字が無く区切りも無い（第2パス）
        (
            "東急田園都市線/溝の口 徒歩15分南武線/津田山 徒歩12分東急田園都市線/梶が谷 徒歩19分",
            {"溝の口": 15, "津田山": 12, "梶が谷": 19},
        ),
        # 賃貸EX: 区切り無しで連結
        (
            "矢野口駅 歩8分稲城長沼駅 歩18分京王よみうりランド駅 歩19分",
            {"矢野口": 8, "稲城長沼": 18, "京王よみうりランド": 19},
        ),
        # ⚠ 同じ駅が2回出て片方がバス便。駅徒歩の方を採る
        (
            "ＪＲ常磐線/柏駅 歩17分東武野田線/柏駅 バス5分 (バス停)「葉山」停留所 歩5分"
            "ＪＲ常磐線/北柏駅 歩21分",
            {"柏": 17, "北柏": 21},
        ),
    ],
)
def test_駅徒歩だけを切り出しバス停からの徒歩は採らない(
    info: str, expected: dict[str, int]
) -> None:
    assert station_walk_minutes(info) == expected


def test_駅表記が無ければ空を返す() -> None:
    assert station_walk_minutes(None) == {}
    assert station_walk_minutes("") == {}
