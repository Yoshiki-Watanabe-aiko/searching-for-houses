...
| CHINTAI_EX | 賃貸EX | ✅ 実装済み（Phase 5 で**本採用**。45分圏でユニーク率63% → 課題#5） |
...
- 実運用: **エリア帯ごとに1本**（Git管理下。課題#9 解消）
  ...
  - [`configs/chintai_suburb45.yaml`](../configs/chintai_suburb45.yaml) — 近郊45分圏（63市区）
...
|---|---:|---:|---|
| 東京23区賃貸 | 23 | **120,000**（暫定） | 70,000〜120,000 |
| 近郊45分圏賃貸 | 63 | 70,000 | 50,000〜70,000 |
...
- ⚠ **帯1の 120,000 は暫定値。** 7万円では23区に足立区19件・葛飾区11件しか無い。
  広めに取ってから `rescore` で締める（締める方向はネットワーク不要のDBバッチで
  試せるが、緩める方向は取り直しになる。MUST 1段目で `fail` の掲載はDBにも入らない）
