...
- ⚠ **パーサを直しても既存データは直らないことがある。** `upsert_listings` は
  `address = COALESCE(EXCLUDED.address, t_listings.address)` の形で更新するので、
  **修正の結果その項目が NULL になるケースでは古い誤った値が残り続ける**。
  さらに詳細ページ由来の項目は詳細キュー（`detail_fetched_at IS NULL`）に乗らないため
  **二度と再取得されない**。⚠ 設備（`re-extract`）・経路（`re-segment`）と違い、
  **住所・交通欄には保存済み原文から作り直す仕組みが無い**。
  直すには該当行の `address` と `detail_fetched_at` を NULL にして取り直させる
  （2026-09-05・ホームメイトの住所6件で実際に踏んだ → 課題#44）
