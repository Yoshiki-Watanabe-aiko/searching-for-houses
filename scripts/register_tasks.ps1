...
#   HouseSearch-Sweep      毎週日曜 02:35      在庫棚卸し（全パターン・一覧5ページ＋詳細400件/サイト）
...
$Tasks = @(
    ...
    @{
        ...
        TaskArg     = "sweep"
        Scraping    = $true
        # ⚠⚠ **増分スキャンの走行帯（:15〜約:27）に重ねてはいけない。**
        # 02:00 起点だった間、01:15 起点の増分（実測約72分＝02:27頃まで）と必ず重なり、
        # 取得ロックが取れずに **2回とも1行もスキャンせず終了コード0で終わっていた**
        # （logs/task_sweep_20260906_020000.err.log・同 20260913_020001。本文は
        # 「scan: 他の取得処理が実行中のためスキップしました。」の1行だけ）。
        # ⚠ **タスクの結果は 0（成功）・ログも1行なので気づけない。** 実害は
        # `t_scrape_runs` に `mode='full'` の行が**全サイト・全パターンで0件**という形で残り、
        # 「週に一度は5ページ舐める」という設計が一度も実行されていなかった（→ 課題#69）。
        # 02:35 なら増分の終了後に開始でき、ロックは自分が先に取る（以降の増分が
        # スキップされるのは設計どおり → ADR 0013 決定8）
        StartAt     = "2026-09-06T02:35:00"
        Repeat      = $null
        Weekly      = $true
        TimeLimit   = "PT10H"
    },
