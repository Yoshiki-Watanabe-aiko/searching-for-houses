...
- ⚠⚠ **一覧の UPSERT はファミリが違う既存行を更新しない**（`ON CONFLICT … WHERE` → 課題#61）。
  一意キーが `(site_id, external_id)` なので、無いと戸建ての行を土地の値で上書きし種別は戸建てのまま
  残す。⚠ **同じファミリの新築⇔中古は従来どおり上書きする**——SUUMO は新築と中古で `nc_` を
  使い回す実例がある（本番2件）。どちらの種別に持たせるかは課題#62（ユーザー判断待ち）
