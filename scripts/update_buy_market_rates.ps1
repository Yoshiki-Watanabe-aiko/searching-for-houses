# ============================================================
# 物件検索通知システム v2 - 売買相場の月次更新（課題#49 Step 8）
#
# 使い方:
...
#      data/market_rates/raw/reinfolib/ へ保存
#      （全国47都道府県×4期＝188ファイル・10秒間隔で約31分）
#   2. data/market_rates/buy_rates.csv を作り直す（市区×単価区分の中央値）
#   3. sync-market-rates --buy で m_market_rates へ投入する
...
#     ⚠⚠ **毎月チェックするので空振りのほうが多い**（2026-09-09 に四半期→毎月へ）。
#     公開時期が読めないため、四半期に絞ると最大3ヶ月反映が遅れる。空振りは異常ではない
#   - ⚠ APIキー（MLIT_REINFOLIB_API_KEY）は取得スクリプトが .env から読み、
#     ヘッダでのみ送る。このスクリプトはキーに触らない（ログにも出さない）
#   - ⚠ 家賃相場の更新（毎月1日 04:30・全国で約90分）と同じ日に走らせない。
#     このタスクは毎月「2日」04:30 に置いてある
...
#     そのため。⚠ 毎月チェックになったので差分のノイズは年4回 → 年12回に増える
