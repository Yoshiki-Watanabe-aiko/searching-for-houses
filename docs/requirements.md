...
|---|---|---|
| SUUMO | SUUMO | ✅ 実装済み |
| HOMES | LIFULL HOME'S | ✅ 実装済み。**1回の実行で5リクエストが上限**（間隔を広げても動かない）。市区ローテーション待ち → 課題#17・#36 |
| ATHOME | アットホーム | ✅ 実装済み。**1回の実行で4リクエストが上限**。現在 `is_active=false`、Phase 5E で復帰予定 → 課題#20・#36 |
| NIFTY | ニフティ不動産 | ✅ 実装済み（市区指定必須・他社掲載を集約するポータル）。**405 は解消済み**（2026-09-03 実測・10市区連続成功）。取り直しが要る → 課題#25 |
...
所要時間が伸びるだけの 10秒は **Phase 5E で 2.5秒へ戻す**。
...
- **MUST をサイト側へ渡す軸は `search.site_filters` で指定する**（→ ADR 0015）。
  ...
  **配線済みは SUUMO・HOMES・GOO・APAMAN**（実測 2026-09-03・足立区）。
