...
def features_text(doc) -> str | None:
    """設備の原文を作る。**「特徴ピックアップ」と「設備仕様」の2ブロック**から採る。

    ⚠⚠ **見出しのクラスで絞ってはいけない**（→ 課題#4・2026-09-09）。SUUMO 売買の
    詳細ページには**テンプレートが2種類**あり、同じ物件種別でも
    ``h3.secTitleInnerR`` のページと ``h3.secTitleInnerK`` のページが混在する
    （実測12件：中古一戸建て・中古マンション・新築一戸建てのいずれでも両方あった）。
    ⚠ **これは種別では決まらない**——初版は種別ごとに1件ずつ観測して
    「中古は R・新築マンションは K」と一般化したが誤りで、実データの**多数派は K** だった。

    ⚠⚠ **外すと「原文が空になるだけ」で例外にならず件数も減らない。** 詳細ページ自体は
    読めている（築年月・管理費は付く）ので、実データで測るまで気づけなかった。
    実測（2026-09-08 時点）で原文が付いていたのは中古一戸建て17%・中古マンション23%・
    新築一戸建て44%で、残りは全部このクラス違いだった。設備の weight は
    中古マンション 60/158・戸建て 50/158 なので、**取れない掲載は構造的に沈む**。

    そのためクラスは見ず、**見出しの文言**（SUUMO の項目名なのでクラス名より安定する）で
    拾う。⚠ 誤って別のブロックを拾わないことは ``test_広告のキャッチコピーは…`` と
    ``test_掲載終了ページでは…`` が固定している。
    ...
    """
    parts: list[str] = []
    for heading in doc.cssselect("h3"):
