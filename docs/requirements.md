...
| **HOMEMATE** | **ホームメイト** | ✅ 実装済み（Phase 5H）。市区の検索値が **JIS5桁そのもの**でスラグ収集が要らない。⚠ **robots.txt が `User-agent: ClaudeBot` を全面禁止**しているが、名乗るUAは `house-search/2.0` なので `User-agent: *` の規則が適用される（`*` に従うのはユーザー判断 2026-09-04 → 詳細設計書 §13.2）。⚠ **`ekiw`（駅徒歩）は選択肢が15分までで MUST の20分を表現できないため配線しない**。⚠ 階の表記が無い住戸が実在し、物件番号に全角数字が混じる |
