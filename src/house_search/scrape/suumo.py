...
# 一覧1ページあたりの**建物**数（``pc``）。⚠ 返るのは住戸なので掲載数はこれより多い
# （実測 2026-09-16・足立区で建物50に対し住戸65）。30 から 50 へ上げて一覧の
# リクエスト数を約4割減らした（該当8,939件の最終ページが 32 → 19 → 課題#69）。
# ⚠ **選択肢にない値は黙って無視される**ので、変えるときは実サイトで測る
# （サーバは解釈した値をHTML末尾のコメントへ echo するのでそこを見る）。
PAGE_SIZE = 50
...
class SuumoScraper:
    ...
    def is_last_page(self, count: int) -> bool:
        ...
        この判定はほぼ発火しない（1建物に複数住戸があり、実測で50建物から65住戸）。
        最終ページの検出は ``last_page``（下）が担う。ここは「ページ送りのフックを
        持たないサイトと同じ既定の振る舞い」を残すためだけに置いてある。

        ⚠ 単位が違っても**早すぎる打ち切りにはならない**。住戸数は建物数以上なので、
        ``count < PAGE_SIZE`` が真なら建物数も上限未満＝本当に最終ページである
        （取りこぼす側ではなく、無駄に1ページ多く読む側へ倒れる）。
        """
